...
To invite users to your account, you must specify the email, account role, and the namespace permissions for the user. You can invite multiple emails at once. The account role can be one of `Read`, `Developer`, or `Admin`. The namespace permissions can be specified using the `--namespace-permission` flag. The namespace permissions can be specified as a list of `namespace=permission` pairs. The permission can be one of `Read`, `Write`, or `Admin`. If the user is not an account admin and no namespace permissions are specified, the user will not have access to any namespaces.
```
...
If a user has been invited to your account but has not accepted the invite, you can reinvite them using the `user reinvite` command. This command will send a new invite email to the user.
```

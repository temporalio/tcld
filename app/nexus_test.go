...
func getExampleNexusEndpoint() *nexus.Endpoint {
	return &nexus.Endpoint{
		...
		CreatedTime:      &timestamppb.Timestamp{Seconds: time.Date(time.Now().Year(), time.April, 12, 0, 0, 0, 0, time.UTC).Unix()},
		LastModifiedTime: &timestamppb.Timestamp{Seconds: time.Date(time.Now().Year(), time.April, 14, 0, 0, 0, 0, time.UTC).Unix()},
	}
}
